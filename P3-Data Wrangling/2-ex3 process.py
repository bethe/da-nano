...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for row in rows:
            ...
            if cell[1].text.lower() != "total":
                print cell[0].text, cell[1].text, cell[2].text, cell[3].text
                info['year'] = int(cell[0].text)
                info['month'] = int(cell[1].text)
                info['flights'] = {"domestic": int(cell[2].text.replace(',','')),
                               "international": int(cell[3].text.replace(',',''))}
                data.append(info)
