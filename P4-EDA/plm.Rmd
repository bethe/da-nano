...
Indeed, it looks like players who are 'not_present' or on the 'bench' are almost solely responsible for the many obersvations with 0 earnings. This highlights the importance of picking players that are likely to play. In terms of analyzing the effect of other variables, these observations can be confounding, so we will take them out from here on onwards.
...
If we just look at average and median, the scales adjust and the trend is more clear.
...
An interesting trend emerges looking at value for money by position. It looks like the further up the field, the higher the median value for money. 


#### Explore individual players earnings
...
(1) Many players only played a subset of the matches (i.e. Galvez, Gulselam, Johannsson and Toljan), which again highlights the necessity to take into account the likelihood of players being played on a given matchday.
(2) There is quite some fluctuation in the earnings of players per matchday.Players earn a significant number of points for Clean Sheets, Assists or scoring a goal. Let's look at those variables a bit closer.
...
In this plot, the teams are listed on the x-axis ordered by their current league table standings (via total points). Opponents are listed on the y-axis in the same order. As the plot is clearly more red towards the bottom right corner and more blue towards the top left, this visualizes the point that the stronger a team and the weaker their opponent, the more points players will earn on average.
...
We saw in the individual analysis the role of game_play_duration, individual achievements and club-level achievements in explaining variance in earnings-point per player. Beyond looking at the sample, we'll look at all players to see if this holds true.
...
ggplot(subset(plm.ranked, !is.na(sum.achievements)),
  ...
  geom_line(aes(color = "blue"), stat = 'summary', fun.y = mean) +
  scale_y_sqrt() +
...
3. Players with high average earnings play more often and more minutes (higher density for players with higher ranks)
